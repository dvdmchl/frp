...
describe('ProfileEditForm Component', () => {
  ...
  it('updates personal info', async () => {
    ...
    await waitFor(() => {
      expect(UserManagementService.updateAuthenticatedUserInfo).toHaveBeenCalledWith({
        fullName: 'New Name',
  ...
  it('renders security form and updates password', async () => {
    ...
    await waitFor(() => {
      expect(UserManagementService.changeAuthenticatedUserPassword).toHaveBeenCalledWith({
        oldPassword: 'oldPass',
        newPassword: 'newPass',
      })
