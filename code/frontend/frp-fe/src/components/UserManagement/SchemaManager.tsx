  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <TableBody className="divide-y">
          {schemas.map((schema) => (
            <TableRow key={schema} className="bg-white dark:border-gray-700 dark:bg-gray-800">
              <TableCell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{schema}</TableCell>
              <TableCell>{user?.activeSchema === schema ? '✓' : ''}</TableCell>
              <TableCell>
        ...
        <ModalBody>
          <div className="space-y-6">
            <InputText
              ...
              value={newSchemaName}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewSchemaName(e.target.value)}
            />
            {createError && <ErrorDisplay error={createError} />}
          </div>
        </ModalBody>
        <ModalFooter>
          <Button onClick={handleCreate} disabled={loading || !newSchemaName}>
            {t('schema.create')}
          </Button>
      ...
      <Modal show={showCopyModal} onClose={() => setShowCopyModal(false)}>
        ...
        <ModalBody>
          <div className="space-y-6">
            <div>
              ...
              <Select
                value={copySource}
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCopySource(e.target.value)}
              >
            ...
            <InputText
              ...
              value={copyTarget}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCopyTarget(e.target.value)}
        ...
        <ModalFooter>
          <Button onClick={handleCopy} disabled={loading || !copySource || !copyTarget}>
            {t('schema.copy')}
          </Button>
